...
if (empty($required_date) || empty($purpose) || empty($items)) {
    echo json_encode(['success' => false, 'message' => 'Required date, purpose, and at least one item are required']);
    exit();
}
...
foreach ($items as $item) {
    if (!empty($item['type_id']) && !empty($item['quantity']) && $item['quantity'] > 0) {
        $valid_items[] = [
...
if (empty($valid_items)) {
    echo json_encode(['success' => false, 'message' => 'At least one valid item is required']);
    exit();
}
...
try {
    ...
    foreach ($valid_items as $item) {
        // Get estimated value for the item type
